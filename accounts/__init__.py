...
def get_backend(path):
    """
        Return an instance of an authentication backend, given the dotted
    ...
    try:
        ...
        raise ImproperlyConfigured('Error loading authentication backend %s: "%s"' % (module, e))
    try:
        backend_class = getattr(mod, attr)
    except AttributeError:
        raise ImproperlyConfigured('Module "%s" does not define an authentication backend named "%s"' % (module, attr))
